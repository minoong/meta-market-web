...
import pipe1 from '~/utils/lmw/pipe1'
import Arrow from '~/utils/lmw/types/Arrow'
import IterableInfer from '~/utils/lmw/types/IterableInfer'
import ReturnValueType from '~/utils/lmw/types/ReturnValueType'
import { isAsyncIterable, isIterable } from '~/utils/lmw/_internal/utils'
...
async function async<A, B>(f: (a: B, b: A) => B, acc: Promise<B>, iterable: AsyncIterable<A>) {
...
function reduce<A, B>(f: (a: B, b: A) => B | Promise<B>, seed: B | Promise<B>, iterable: AsyncIterable<A>): Promise<B>

function reduce<A, B>(f: (a: B, b: A) => B | Promise<B>, iterable: AsyncIterable<A>): Promise<B>

function reduce<A extends Iterable<unknown> | AsyncIterable<unknown>>(
 f: (a: IterableInfer<A>, b: IterableInfer<A>) => IterableInfer<A> | Promise<IterableInfer<A>>,
...
): B | undefined | Promise<B | undefined> | ((iterable: A) => ReturnValueType<A, B>) {
 if (iterable === undefined) {
  if (seed === undefined) {
   return (iterable: A) => reduce(f, iterable as any) as ReturnValueType<A, B>
  }
