import { Reject, Resolve } from '~/utils/lmw/types/Utils'
import { isAsyncIterable } from '~/utils/lmw/_internal/utils'
...
function concurrent<A>(
 ...
 const fillBuffer = () => {
  if (pending) {
   ...
   const nextItems = Promise.allSettled(Array.from({ length }, () => iterator.next(Concurrent.of(length) as any)))
   pending = true
   prev = prev
    .then(() => nextItems)
    .then((nextItems) => {
