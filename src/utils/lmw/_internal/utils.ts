import { Iter } from '~/utils/lmw/types/Utils'

export function isIterable<T = unknown>(a: Iter<T> | unknown): a is Iterable<T> {
 return typeof (a as any)?.[Symbol.iterator] === 'function'
}

export function isAsyncIterable<T = unknown>(a: Iter<T> | unknown): a is AsyncIterable<T> {
...
export const isNotNullable = <T>(a: T): a is NonNullable<T> => a !== null && a !== undefined
